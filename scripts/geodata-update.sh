...
MOSDNS_PATH=/usr/local/etc/mosdns
...
unzip -qq -o $MOSDNS_PATH/downloads/geoip.zip -d $MOSDNS_PATH/tmp/ips
echo "Extracting geosite.zip to domains/ ..."
unzip -qq -o $MOSDNS_PATH/downloads/geosite.zip -d $MOSDNS_PATH/tmp/domains
